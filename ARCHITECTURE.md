...
- **Which account sends** (ADR-37, ADR-38) — one definition, `app.salon_messaging_state()`:
  `own` (the salon's account) → `trial` or `grace` (Crayora's, on purpose, not alerted) →
  `blocked` (a sponsorship existed, all of it has ended, still no own account: **nothing is sent**)
  → `fallback` (never sponsored: Crayora's as an alerted fault). A code is always validated against
  the account that sent it.
- **Blocked** (ADR-38) is enforced at three points — `resolve_join_code`/`start_join`, `otp_begin`,
  and `app.salon_writable()`, which every INSERT/UPDATE policy calls — and deliberately at none of
  the read policies, and not on consent withdrawal.
